package test.boardqna.q.controller;
...
import test.boardqna.q.model.service.BoardQnaQService;
import test.boardqna.q.model.vo.BoardQnaQ;
import test.client.model.service.ClientService;

public class BoardQnaQController {
	private BoardQnaQService bqs = new BoardQnaQService();
	...
	public void createDummy() {
		...
		for (int i = 0; i < 100; i++) {
			...
			temp.setqUser(idList.get((int) (Math.random() * 99)));
			temp.setTitle(randomHangulName());
			temp.setEtc(randomHangulName() + " " + randomHangulName());
			temp.setType((int) (Math.random() * 3));

			if (bqs.createDummy(temp) == 1) {
				System.out.println("| LOG | CreateBoardQna_Q Dummy | " + i + " | " + temp.toString());
			}
		}
